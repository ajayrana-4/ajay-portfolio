...
export default function SkillsSection({ data }: { data: Skill[] }) {
  ...
  return (
    <section id="skills" className="section" style={{ background: 'linear-gradient(to bottom, var(--bg-primary), var(--bg-secondary), var(--bg-primary))' }}>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <div key={cat} className="glass-card p-6">
                <div className={`inline-flex items-center px-3 py-1.5 rounded-lg bg-gradient-to-r ${colorClass} border text-xs font-semibold mb-5`}>
                  {cat}
                </div>
